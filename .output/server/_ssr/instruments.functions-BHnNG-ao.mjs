...
//#region node_modules/.nitro/vite/services/ssr/assets/instruments.functions-BHnNG-ao.js
...
var getUniverseStats = createServerFn({ method: "GET" }).handler(getUniverseStats_createServerFn_handler, async () => {
	const { universe } = await import("./instruments.server-BLWYFv13.mjs");
...
var searchListedStocks = createServerFn({ method: "GET" }).inputValidator((input) => objectType({ query: stringType().min(1).max(40) }).parse(input)).handler(searchListedStocks_createServerFn_handler, async ({ data }) => {
	const { searchUniverse, livePrices } = await import("./instruments.server-BLWYFv13.mjs");
...
var getListedQuote = createServerFn({ method: "GET" }).inputValidator((input) => objectType({ symbol: stringType().min(1).max(24) }).parse(input)).handler(getListedQuote_createServerFn_handler, async ({ data }) => {
	const { resolveInstrument, livePrice } = await import("./instruments.server-BLWYFv13.mjs");
