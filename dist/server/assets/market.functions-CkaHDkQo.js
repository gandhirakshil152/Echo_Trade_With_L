...
import { z } from "zod";
//#region src/lib/market.functions.ts?tss-serverfn-split
...
var getCandles = createServerFn({ method: "GET" }).inputValidator((input) => z.object({
	...
	const { loadCandles } = await import("./market.server-D0d7uVjJ.js");
...
var getFundamentals = createServerFn({ method: "GET" }).inputValidator((input) => z.object({ symbol: z.string().min(1).max(20) }).parse(input)).handler(getFundamentals_createServerFn_handler, async ({ data }) => {
	const { loadFundamentals } = await import("./market.server-D0d7uVjJ.js");
...
var getNews = createServerFn({ method: "GET" }).inputValidator((input) => z.object({ symbol: z.string().max(20).optional() }).parse(input ?? {})).handler(getNews_createServerFn_handler, async ({ data }) => {
	const { loadNews } = await import("./market.server-D0d7uVjJ.js");
